...
use crate::passes::reg_alloc::ast::{RegAllocGraph, RegAllocPredecessors, Possibility, reg_alloc_graph_from_lir};
...
pub struct RegAllocError<Source> where Source: 'static + error::Error {
...
fn find_fixpoint(graph: &mut RegAllocGraph, strict_meet: bool) -> Result<(), RegAllocError<RegAllocFixpointError>> {
    /*
    ...
    */
    ...
    let mut dirty = graph.iter().enumerate().filter_map(|(raw_node_index, node)| {
    ...
    loop {
        ...
        */
        ...
        let state_is_complete = match node.operation.judge(&state) {
            ...
            Possibility::Invalid => {
                return Err(RegAllocError { node_index, source: RegAllocFixpointError { state: state.clone() } });
            }
        };
...
fn check_graph_completeness(graph: &RegAllocGraph) -> Result<(), RegAllocError<RegAllocCompletenessError>> {
